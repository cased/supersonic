...
class DiffParser:
    ...
    def parse_detailed(self, diff_content: str) -> List[Tuple[FileDiff, List[DiffHunk]]]:
        """
    ...
    def generate_commit_message(self, diffs: List[FileDiff]) -> str:
        ...
        if len(diffs) == 1:
                ...
                if diff.new_content:
                    ...
                    for line in lines:
                        if any(pattern in line.lower() for pattern in 
                              ('def ', 'class ', 'function', 'interface')):
                            return f"Update {file_type}: {line.strip()}"
            
            # Default single file message
            action = "Add" if diff.is_new_file else "Delete" if diff.is_deletion else "Update"
            ...
            counts = {
                ...
                'modified': len([d for d in diffs if not (d.is_new_file or d.is_deletion)])
            }
            ...
            parts = []
            if counts['added']: parts.append(f"Add {counts['added']} files")
            if counts['deleted']: parts.append(f"Delete {counts['deleted']} files")
            if counts['modified']: parts.append(f"Update {counts['modified']} files")
            return ", ".join(parts)
