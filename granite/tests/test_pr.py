...
def mock_github():
    ...
    mock.create_pull_request = AsyncMock(return_value="https://github.com/test/test/pull/1")
...
async def test_create_pr_from_files(mock_github):
    ...
    files = {
        "src/test1.py": "print('test1')",
        "src/test2.py": "print('test2')"
    }
    ...
    assert any(call[1]["path"] == "src/test1.py" and call[1]["content"] == "print('test1')" for call in calls)
    assert any(call[1]["path"] == "src/test2.py" and call[1]["content"] == "print('test2')" for call in calls)
